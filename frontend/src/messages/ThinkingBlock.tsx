...
export function ThinkingBlock({ content, streaming }: ThinkingBlockProps) {
  ...
  return (
    <div
      className={`thinking${open ? "" : " collapsed"} mb-3.5 py-0.5 text-[13px] leading-[1.6] text-fg-muted`}
    >
      <div
      ...
      <div
        className={`mt-1.5 max-h-[360px] overflow-y-auto text-[13px] whitespace-pre-wrap text-fg-muted [scrollbar-gutter:stable]${open ? "" : " hidden"}`}
      >
        {content}
      </div>
    </div>

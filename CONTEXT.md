...
**目录归档（Catalog Archive）**:
把聊天模型、模型路由或模型上游移出可用目录的可逆状态。归档行保留且只读，释放自然 key 供同名重建，只能按行句柄 `ref` 显式恢复；归档模型会一并归档其路由。已创建的 Run 按快照中的路由继续使用当时那一行上游，不受归档影响。
_Avoid_: 删除模型、下线（下线是 `enabled=false`，不释放 key）、软删除

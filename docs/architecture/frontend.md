...
- 分享有两条入口且语义不同：聊天页浮动分享是一步操作——复用已生效链接、否则新建永久链接，
  ...
  生效链接，任何新入口都必须先复用再创建，不能靠 409 兜底。安全上下文中的快捷分享必须在首个
  异步请求前启动可延迟取值的 ClipboardItem 写入，以保留 WebKit 用户手势。非安全上下文只能尝试
  已弃用的选区复制，该路径在真实移动浏览器经过异步请求后不可靠，该局域网 HTTP 场景不属于本次支持目标，生产 HTTPS 的补丁与复测要求见
  `docs/handover/frontend/2026-09-05-mobile-quick-share-copy-failure.md`。桌面和移动端都不因浏览器形态
  切换为分享对话框，且只有真实复制成功后才能显示成功提示。
...
- `/model-admin` 是单页运维入口，布局为左侧模型/上游列表（搜索、已归档入口）+ 右侧所选项
  详情；窄屏从列表钻取到详情并可返回。解锁后每次写操作都以服务端返回的完整目录替换本地镜像，
  不在前端乐观推演路由选择。
...
- 聊天模型 key、模型上游 key 和既有路由三元组是稳定身份，Web 编辑时不可原地改名；需要
  更换身份时先新增、验证，再下线旧项。Web 与 CLI 均不提供 hard delete，移除使用可逆的归档
  （ADR 0014）。归档后同一 key 可以重建，因此归档项与同名活跃项可能并存：列表 key、分组与恢复
  一律使用服务端返回的 `ref`，路由通过 `model_ref` / `upstream_ref` 归属父项，不得按 key 推断。

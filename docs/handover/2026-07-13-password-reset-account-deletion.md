...
> **后续状态（2026-08-01）**：本期的“注销 = 软停用”结论仍有效。统一 files 领域补充了
> 在途上传取消、拒绝新附件读取 URL、私有已绑定附件保留，以及公开头像立即 delete+purge
> 的具体执行边界；以 [统一文件上传交接](2026-08-01-unified-file-upload.md) 为当前权威。
> 这不表示已引入账户级 30 天物理清除，也不改变不得借道密码重置恢复账户的约束。

...
- 桌面侧栏使用「固定宽度 280px 内容画布 + 可变宽度裁切外壳」。收起时 `<aside>` 的右边界从
  280px 推进到 52px，完整面板、会话行和底部账号触发器不改变自身坐标；完整导航内容层使用与
  外壳完全同步的 220ms `cubic-bezier(0.4, 0, 0.2, 1)` 透明度过渡（无 `filter` 模糊），并固定横向
  裁切与不换行。文字与元素从点击后的第一批动画帧便和外壳同时渐隐，不会在宽度接近终点时才补做
  淡出，也不会因文字重排产生挤压感；`prefers-reduced-motion` 下禁用过渡。
- rail 控件层使用方向性切换时序：收起时 `steps(1,start)` 立即接管左侧 52px；展开时在 150ms 内
  与完整内容层同步线性渐隐。外壳继续由 52px 向 280px 扩展，形成从左到右的连续揭示，不会等侧栏
  完全展开后才突然移除 mini rail。
...
- 收起点击后的首个可测动画帧中，外壳宽度已由 280px 变为约 276.8px，完整内容透明度也由 1 变为
  约 0.986；其后宽度与透明度始终按同一 220ms 曲线推进，没有末段才开始淡出的停顿；

...
- 前端 CI：执行 `pnpm install --frozen-lockfile`、`pnpm run lint`、`pnpm run typecheck`、`pnpm run test`、`pnpm run build`。
...
- `frontend` 可独立执行 `pnpm install --frozen-lockfile`、`pnpm run test -- --run`、`pnpm run build`。
